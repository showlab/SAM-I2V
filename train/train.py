...
def single_proc_run(local_rank, main_port, cfg, world_size):
    ...
    if local_rank == 0:
        print(f"Total training time: {int(days)} day {int(hours)} hour {int(minutes)} minute")

    print(f"Saved path: {cfg.launcher.experiment_log_dir}")
...
def main(args) -> None:
    ...
    if cfg.launcher.experiment_log_dir is None:
        cfg.launcher.experiment_log_dir = os.path.join(
            os.getcwd(), "i2v_logs", args.config
        )
    ...
    if submitit_conf.use_cluster:
        ...
        if cfg.launcher.num_nodes == 1:
            main_port = random.randint(
                submitit_conf.port_range[0], submitit_conf.port_range[1]
            )
            single_node_runner(cfg, main_port)
        else:
            multi_node_runner(cfg)


if __name__ == "__main__":

    initialize_config_module("i2v", version_base="1.2")
