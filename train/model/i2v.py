...
from i2v.modeling.i2v_base_train import I2VBase

from i2v.modeling.i2v_utils import (
    get_1d_sine_pe,
    get_next_point,
    sample_box_points,
...
from i2v.utils.misc import concat_points
import sys
import os
...
class I2VTrain(I2VBase):
    def __init__(
        ...
        # During train, we optionally allow sampling the correction points from GT regions
        # instead of the prediction error regions with a small probability. This might allow the
        # model to overfit less to the error regions in train datasets
    ...
    def forward(self, input: BatchedVideoDatapoint):
        if self.training or not self.forward_backbone_per_frame_for_eval:
            backbone_out = self.forward_image_video(input.flat_img_batch,
                                                    input.flat_vid_batch(self.image_encoder.trunk_video.t_window))
        else:
    ...
    def prepare_prompt_inputs(self, backbone_out, input, start_frame_idx=0):
        ...
        if num_frames == 1:
            # here we handle a special case for mixing video + SAM on image train,
        ...
        for t in init_cond_frames:
            if not use_pt_input:
                ...
                # During train # P(box) = prob_to_use_pt_input * prob_to_use_box_input
